 ...
 * Comprehensive AutoML with dataset selection, job tracking, and results
 */
...
export function AutoMLEngine() {
  ...
  const [autoMLConfigOpen, setAutoMLConfigOpen] = useState(false);
  ...
  // Debug: Log the training data groups (REMOVED - was causing console flooding)
  // console.log('🔍 Training Data Groups:', JSON.stringify(trainingDataGroups, null, 2));
  // console.log('📊 Datasets:', datasets);
  // console.log('📊 Data Sources:', dataSources);
  // console.log('📊 All Training Data:', allTrainingData);
  ...
  const handleStartAutoML = async () => {
    ...
    try {
      ...
      const response = await automlService.startAutoMLJob({
        ...
        name: jobName || `AutoML Run - ${new Date().toLocaleString()}`,
        targetColumn,
        problemType,
      ...
      toast.success('AutoML job started successfully!');
      ...
      toast.error(error.message || 'Failed to start AutoML');
    } finally {
      setStarting(false);
    }
  };
  ...
  const handleJobComplete = useCallback((jobId: string) => {
    ...
    setShowResultsModal(true);
    refetchJobs();
    refreshCurrentProject(); // ← Refresh project stats after AutoML completes
  ...
  const handleStopJob = async (jobId: string) => {
    try {
      ...
      toast.success('AutoML job stopped');
      setShowProgressModal(false);
      refetchJobs();
    } catch (error: any) {
    ...
    return (
      <div className="flex items-center justify-center h-[600px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Select a project to use AutoML</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Automatic machine learning - let AI build the best model for you
          </p>
        </div>
        <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="AutoML Run - Today"
            />
          </div>
        </div>
      </Card>

      {/* AutoML Configuration - COLLAPSIBLE */}
      <Collapsible open={autoMLConfigOpen} onOpenChange={setAutoMLConfigOpen}>
        <Card className="p-6">
          <CollapsibleTrigger className="flex items-center justify-between w-full hover:opacity-80 transition-opacity">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-lg">AutoML Configuration</h3>
            </div>
            <div className="flex items-center gap-2">
              {!autoMLConfigOpen && (
      ...
      {jobs.length > 0 ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            <h3 className="font-semibold text-lg">AutoML History</h3>
        ...
        <Card className="p-8 bg-gradient-to-br from-slate-50/50 to-gray-50/50 dark:from-slate-900/50 dark:to-gray-900/50">
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm mb-6 max-w-md mx-auto">
              Configure your training data and settings above, then click "Start AutoML" to train your first model.
            </p>
